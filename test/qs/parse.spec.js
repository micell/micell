...
describe('qs/parse', () => {
  ...
  it('multiple query pairs', () => {
    ...
    expect(parse('foo=1&bar=2&baz=3')).to.deep.equal({ foo: '1', bar: '2', baz: '3' })
  })
  ...
  it('should decode query name and value', () => {
    ...
    expect(parse('foo%20bar=hello%20world')).to.deep.equal({ 'foo bar': 'hello world' })
  })

  it('multiple query pairs with the same name', () => {
    expect(parse('lang[]')).to.deep.equal({ lang: [''] })
    expect(parse('lang[]=')).to.deep.equal({ lang: [''] })
    expect(parse('lang=HTML&lang=CSS&lang=JavaScript')).to.deep.equal({ lang: ['HTML', 'CSS', 'JavaScript'] })
    expect(parse('lang[]=HTML&lang[]=CSS&lang[]=JavaScript')).to.deep.equal({ lang: ['HTML', 'CSS', 'JavaScript'] })
  })
})
